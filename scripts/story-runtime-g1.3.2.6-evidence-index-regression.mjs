// G1.3.2.6 evidence-index regression：P2-1 ——
// - 验证 `2026-08-09-g1.3.2.6-evidence/` 的 detached manifest 自洽（排除自身；条数/大小/hash 逐文件复验）；
...
async function main() {
  const EV = 'docs/superpowers/specs/2026-08-09-g1.3.2.6-evidence';
  const REPORT = 'docs/superpowers/specs/2026-08-09-story-composition-v3-g1.3.2.6-report.md';
  const safety = [];
  const positives = [];
  const rejections = [];
  ...
  // ══ 场景 1：detached manifest 自洽 ══
  {
    ...
    assert(JSON.stringify(actualFiles) === JSON.stringify(indexed), '场景1-目录文件与 manifest 完全一致（缺失: ' + JSON.stringify(actualFiles.filter((f) => !manifestNames.has(f))) + '）');
  ...
  // ══ 场景 2：summary 元数据 ══
  {
    ...
    assert(lines.length >= 45, '场景2-至少 45 条命令记录，实际 ' + lines.length);
  ...
  console.log('story-runtime-g1.3.2.6-evidence-index regression passed.');
...
if (isMain) {
  main().catch((error) => {
    console.error('story-runtime-g1.3.2.6-evidence-index regression failed: ' + (error instanceof Error ? error.message : String(error)));
